...
def recolte_sang(N,K,Q,S_min,alpha,locations,dispo,debug):
    ...
    if model.status == GRB.OPTIMAL:
        ...
        if debug:
    # Définir une liste de couleurs ou une carte de couleurs
            colors = plt.cm.tab10(np.linspace(0, 1, K))  # Utilise une palette de 10 couleurs distinctes
            ...
            for k in range(K):
                if z[k].X > 0.5:  # Vérifie si le camion k est utilisé
                    for i in range(N):
                        for j in range(N):
                            if x[i, j, k].X > 0.5:  # Vérifie si le camion k passe de i à j
                                plt.plot(
                                    [locations[i, 0], locations[j, 0]],
                                    [locations[i, 1], locations[j, 1]],
                                    color=colors[k],  # Couleur unique pour chaque camion
